...
$(document).ready(function () {
  ...
  $(".more").click(function() {
    ...
    if($(this).hasClass("list-flat__link")){
      ...
      for(let i = 0; i < 3; i++){
        let cloned = $(elements[i]).clone(true)[0];
        cloned.addEventListener("click", (e) => popupTable(e));
        $(".tbody").children(":last-child").after(cloned);
      }
    } else {
      elements = Array.from($(".news-list").children());
      for(let i = 0; i < 3; i++)
        $(".news-list")
          .children(":nth-last-child(2)")
          .after($(elements[i]).clone());
    }
  })  
